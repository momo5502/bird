...
gl_object gl_bufferer::create_texture()
{
    ...
    return {texture, [this](const GLuint obj) { this->schedule_texture_cleanup(obj); }};
}
...
gl_object gl_bufferer::create_buffer()
{
    ...
    return {buffer, [this](const GLuint obj) { this->schedule_buffer_cleanup(obj); }};
}
...
{
    this->buffers_.access([buffer](object_vector& vector) { vector.push_back(buffer); });
}

void gl_bufferer::schedule_texture_cleanup(const GLuint texture)
{
    this->textures_.access([texture](object_vector& vector) { vector.push_back(texture); });
}
