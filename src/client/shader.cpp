...
namespace
{
    ...
    void compile_shader(const GLuint shader, const std::string_view& code, const std::string_view& fixup, const bool apply_fixup)
    {
        if (apply_fixup)
        {
    ...
    {
        return utils::finally([shader] { glDeleteShader(shader); });
    }

    GLuint create_shader_program(const std::string_view& vertex_shader_code, const std::string_view& fragment_shader_code,
                                 const bool apply_fixups)
    {
