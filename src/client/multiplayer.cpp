...
namespace
{
    ...
    void update_body(JPH::PhysicsSystem& system, const JPH::Body& body, const glm::dvec3& position, const glm::dvec3& orientation)
    {
        (void)orientation;
        ...
        system.GetBodyInterface().SetPositionAndRotation(body.GetID(), position_vector, quat.Normalized(), JPH::EActivation::Activate);
    }
    ...
    JPH::Body* create_body(JPH::PhysicsSystem& system)
    {
        ...
        const JPH::BodyCreationSettings body_settings(standingShape.Get(), JPH::DVec3{}, JPH::Quat::sIdentity(), JPH::EMotionType::Static,
                                                      Layers::NON_MOVING);
...
{
    this->manager_.on(
        "states", [this](const network::address& address, const std::string_view& data) { this->receive_player_states(address, data); });

    this->manager_.on("authRequest",
                      [this](const network::address& address, const std::string_view& data) { this->receive_auth_request(address, data); });

    this->manager_.on(
        "killed", [this](const network::address& address, const std::string_view& data) { this->receive_killed_command(address, data); });
}
...
{
    return this->players_.access<size_t>([](const players& p) { return p.size(); });
}
