...
namespace network
{
	...
	{
		thread_ = utils::thread::create_named_jthread("Network Dispatcher", [this](const utils::thread::stop_token& stop_token)
		{
			this->packet_receiver(stop_token);
		});
	}

	void manager::packet_receiver(const utils::thread::stop_token& stop_token)
	{
